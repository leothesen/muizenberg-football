...
/**
 * Where a bot username comes from, in the order it is trusted.
 *
 * An environment variable first, so a rename can be forced without waiting for
 * anything; then what was learnt last time; then Telegram itself. Injected rather
 * than imported so this stays testable without a database.
 */
export interface BotIdentityStore {
  cached(): Promise<string | null>;
  remember(username: string): Promise<void>;
}

/**
 * The bot's own @username.
 *
 * Every route that wants to hand somebody a link into a private chat needs this, and
 * the cost of not having it is invisible: `startDeepLink` is only built when a
 * username is present, so a missing one removes the button rather than breaking
 * anything, and the group quietly loses the one path that makes post-match DMs
 * possible. It went unnoticed for months.
 *
 * So it is asked for rather than configured. `getMe` is called at most once per
 * deployment — the answer is written down, and every request after that reads it
 * back — and a Telegram outage costs a button rather than the whole update.
 */
export async function resolveBotUsername(
  client: Pick<TelegramClient, "getMe">,
  store: BotIdentityStore,
  configured?: string,
): Promise<string | undefined> {
  if (configured) return configured;

  const known = await store.cached();
  if (known) return known;

  try {
    const me = await client.getMe();
    if (!me.username) return undefined;
    await store.remember(me.username);
    return me.username;
  } catch {
    // Without a username the welcome loses a button. Without this catch, an update
    // from somebody saying hello would fail outright.
    return undefined;
  }
}
