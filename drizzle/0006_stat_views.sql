...
--
-- They run as their owner on purpose — a Postgres view does that by default, which
-- is what lets `web_reader` read through them while holding no privilege at all on
-- the tables underneath. Base tables are locked down completely (see the grants
-- migration) and these curated views are the only surface `web_reader` can read,
-- which is also why none of them expose Telegram identifiers.
